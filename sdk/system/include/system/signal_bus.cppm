...
// Fixed-capacity event ring with a drop-oldest overflow policy. At Depth == 1
// it degenerates into the single coalescing slot ("last value wins"); at Depth
// > 1 it preserves FIFO order and drops the oldest event on overflow. Kept free
// of the executor so its ordering can be checked in a consteval self-test.
...
static_assert(
    eventRingSelfCheck(),
    "Channel event ring FIFO/coalesce/drop-oldest broken"
);
...
public:
  ...
  explicit Channel(SingleThreadExecutor &exec)
      : _exec(exec), _item(&dispatchThunk, this) {}
...
private:
  ...
  static void dispatchThunk(void *self) {
    static_cast<Channel *>(self)->dispatch();
  }
