...
// Thin, type-safe wrapper over SingleThreadExecutor's deferred/periodic posting.
// A Timer owns a single WorkItem and delegates scheduling to the executor's
// runLoop; the callback is a captureless thunk (void(*)(void*)), never
// std::function. Handlers run on the executor task, so they are serialized with
// every other posted work item -- no extra locking inside the callback.
class Timer {
public:
