...
constexpr void
insertSorted(system::WorkItem *&head, system::WorkItem &node, bool priority);
...
insertSorted(system::WorkItem *&head, system::WorkItem &node, bool priority) {
  ...
  while (*link != nullptr) {
    const bool keepWalking = priority
                                 ? tickLess((*link)->_dueTick, node._dueTick)
...
constexpr bool unlink(system::WorkItem *&head, system::WorkItem &node) {
  for (system::WorkItem **link = &head; *link != nullptr;
       link = &(*link)->_next) {
    if (*link == &node) {
      *link = node._next;
      node._next = nullptr;
...
public:
  ...
  void scheduleAt(WorkItem &item, uint32_t dueTick, bool priority) {
    armAt(item, dueTick, priority);
  }
...
static_assert(
    workQueueOrderSelfCheck(),
    "WorkQueue FIFO/priority/cancel ordering broken"
);
