...
[![Build](https://github.com/khosta77/stm32-sdk/actions/workflows/build.yml/badge.svg?branch=develop)](https://github.com/khosta77/stm32-sdk/actions/workflows/build.yml)
[![Docs](https://github.com/khosta77/stm32-sdk/actions/workflows/docs.yml/badge.svg?branch=develop)](https://github.com/khosta77/stm32-sdk/actions/workflows/docs.yml)
...
Builds are gated by `-Werror` with `-Wall -Wextra -Wpedantic -Wshadow` —
both for the SDK itself and for every project generated by `stmtool`. See
[Build flags](https://khosta77.github.io/stm32-sdk/build-flags/) for the policy.

## Quick Start

`stmtool` — the CLI that scaffolds, builds, and flashes projects — lives in its
own repository ([`khosta77/stmtool`](https://github.com/khosta77/stmtool)) since
v0.2.1. Install it with pipx:

```bash
pipx install git+https://github.com/khosta77/stmtool.git
```

Or run `./install.sh`, which pins to the latest released tag and configures shell
...
| `stmtool show-version` | Show stmtool version |
...
Starting with v0.1.4 only `STM32F407VG` is validated in CI (the maintainer's
physical hardware). F401/F411 etc. remain supported by the SDK code but
...
still be refined. The single source of versioning is git tags. Release notes live
in `docs/release.md`. `stmtool` is versioned independently in its
[own repository](https://github.com/khosta77/stmtool) (`v0.N`, auto-bumped by CI)
since v0.2.1.
