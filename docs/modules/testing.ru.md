...
`host-tests`. Набор сейчас покрывает `Result<T>` / `DRV_TRY` (`test_result`),
мок-шины (`test_mock`), пример MPU6050-на-моке (`test_sensor_mock`) и оформление
/ фильтрацию / форматирование [логгера](logging.md) (`test_log`). Чтобы добавить
случай, положите `test_*.cpp` в `tests/host/` и зарегистрируйте его в
`tests/host/CMakeLists.txt` через `add_host_test(...)`.
