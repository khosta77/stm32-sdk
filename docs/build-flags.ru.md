...
## Флаги логирования (v0.1.14)

[Логгер](modules/logging.md) поставляется вместе с `stm32_drivers` и настраивается
двумя cache-переменными, отображаемыми в compile-define на цели драйверов:

- `STM32_LOG_LEVEL` (`NONE`/`ERROR`/`WARN`/`INFO`/`DEBUG`/`TRACE`, по умолчанию
  `INFO`) — потолок на этапе компиляции. `LOG_*` выше него раскрываются в
  `((void)0)`, поэтому их строки-форматы не попадают во flash. Поставьте `NONE` в
  релизной сборке, чтобы вырезать логирование целиком.
- `STM32_LOG_BACKEND` (`none`/`itm`/`uart`, по умолчанию `none`) — называет
  предполагаемый sink. Оба backend-модуля компилируются всегда, поэтому флаг не
  ограничивает доступность; он задаёт define `STM32_LOG_BACKEND` (со значениями
  `STM32_LOG_BACKEND_NONE`/`_ITM`/`_UART`), чтобы код приложения выбрал
  подходящий тип backend'а обобщённо.
