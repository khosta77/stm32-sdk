...
public:
  ...
  void run() {
    while (true) {
      const auto value =
          static_cast<int16_t>(static_cast<int32_t>(_seq % 200) - 100);
...
struct SignalDemo {
  ...
  [[nodiscard]] system::BootReport boot() {
    return system::bootstrap(producer, consumer);
  }
};
...
void producerEntry(void *p) {
  static_cast<decltype(&app.producer)>(p)->run();
}
...
int main() {
  ...
  writeStr(
      workQueueDispatchSelfCheck() ? "workqueue self-check: PASS\r\n"
