...
// Reactive subscriber: no task of its own, wakes on the executor task whenever a
// debounced press lands on the channel.
...
public:
    ...
    void onPress(const ButtonEvent &event) {
        ...
        const int len = snprintf(
            buf, sizeof(buf), "press #%lu @tick=%lu\r\n",
            static_cast<unsigned long>(event.seq), static_cast<unsigned long>(event.tick));
        if (len > 0) {
            (void) _uart.write({reinterpret_cast<const uint8_t *>(buf), static_cast<size_t>(len)});
        }
    }
...
// Owns its own worker task and starts it in onStart() (deferred-start): the task
// is created inside the component, not in main. Tasks created before
...
public:
    ...
    void run() {
        ...
        while (true) {
            ...
            char buf[32];
            const int len =
                snprintf(buf, sizeof(buf), "alive #%lu\r\n", static_cast<unsigned long>(beat));
            if (len > 0) {
                (void) _uart.write(
...
struct ButtonDemo {
    ...
    Button<GpioPin> btn{
        {
            .base = {.name = "button", .criticality = system::Criticality::Common},
            .line = exti({
                .line = 0,
                .port = ExtiPort::A,
    ...
    Reporter<Uart<>, GpioPin> reporter{
        {
            .base = {.name = "report", .criticality = system::Criticality::Common},
        },
